        ...
        <div v-if="reservations.length">
            <div class="reservation" v-for="reservation in reservations" :key="'reservation-' + reservation.rvId">
                ...
                <div v-if="reservation.checkIn" class="checkin-card">
                    ...
                    <!-- 체크아웃 버튼 -->
                    <button type="button" class="btn" style="background-color: #FFD152; color: #FFFFFF; width: 150px;"  @click="checkOutRequest(reservation)">
                        체크아웃 요청
                    </button>
    ...
    methods: {
        fetchReservations() {
            ...
            axios.post('/reservation/myReservations', requestDTO, {
                ...
                // 새로운 예약 목록과 기존 예약 목록을 비교하여 삭제된 예약을 확인하고 제거
                const removedReservations = this.reservations.filter(reservation => !newReservations.some(newReservation => newReservation.rvId === reservation.rvId));
                removedReservations.forEach(removedReservation => {
                    const index = this.reservations.findIndex(item => item.rvId === removedReservation.rvId);
                    if (index !== -1) {
        ...
        checkOutRequest(reservation) {
        const checkOutRequest = {
        rId: reservation.checkIn.room.rid // room id만 사용
        // checkOutStatusEnum: "INPROGRESS",
        // coutDate: reservation.checkIn.cinDate,
        // coutTime: reservation.checkIn.cinTime,
